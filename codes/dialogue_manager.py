...
def build_profile(raw_inputs: dict) -> dict:
    ...
    return {
        ...
        "investment_horizon":str(raw_inputs.get("investment_horizon", "Medium (2-5 years)")).strip(),
...
def run_financial_dialogue(
    ...
    Args:
        ...
        mode:      Model mode — "Mock Mode" or "Local Model Placeholder".
    ...
    try:
        ...
        # Step 2: Build the prompt (system + user turns)
        prompt = build_structured_prompt(clean_profile, task_type, language)
        prompt["task_type"] = task_type  # pass task through so backend can use it
        ...
        # Step 4: Attach a human-readable profile summary
